...
lolipopマネージドクラウドでrails7のAPIモードでgraphql gemを使用して、graphqlAPIを用意します。最終的にできた環境が[これ](https://github.com/yagijin/rails_server)。
...
APIモードがどんなモードでどんなメリットがあるかは、railsguideのAPIモードの説明がわかりやすくてよかったのでおすすめします。
...
graphqlと同時についでにgraphiqlも使用する。Gemfileに以下を追記する。
...
## デプロイを自動化する
デプロイは、[このブログ](https://diary.shu-cream.net/GitHub%20ActionsでRailsアプリをロリポップマネージドクラウドにデプロイする)を参考にほぼコピペして、github　actionでpushされるたびに自動デプロイできるようになった。

自分で追加で加えた変更としては、すでにlolipopマネージドクラウドに上がっているgitリポジトリのデフォルトブランチがmasterで開発しているブランチがmainなので、以下のようにrenameして毎回デプロイしてる。
...
## まとめ
こんなかんじでいつでも壊して遊びながら、railsとgraphqlを学ぶ環境ができたので活用していく。
