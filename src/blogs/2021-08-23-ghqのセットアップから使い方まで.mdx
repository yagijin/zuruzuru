...
リポジトリ管理用のツールの ghq のセットアップ方法から使い方までをまとめます。

ghq[^ghq] はソースコードリポジトリの取得と管理用のツールです。
go get を参考にしたディレクトリ構造をもって、git や svn などの複数の Version Control System(VCS)を意識せずに透過的に扱うことができます。
...
`ghq get リポジトリ`で`git clone リポジトリ`と同じようにでリポジトリを取得できます。

また、`ghq get -p リポジトリ`のように -p オプションを付けると ssh で clone できます。
...
このままではリポジトリを移動したりするのが面倒くさいので、 fzf を導入して簡単に移動できるようにします。

## fzf で応用する

fzf[^fzf] は曖昧検索した結果を標準出力するコマンドです。このコマンドをつかって、ghq で取得したリポジトリを検索します。
...
準公式ドキュメント[^ghqhandbook]の例を参考に ghq のリポジトリ検索の処理を書き出すと以下のようになります。この内容を`.zshrc`に追記してください。
...
zsh 利用時にキー`ctrl + ] `を押すと、以下のように検索して、そのリポジトリの配下のディレクトリに cd できます。
...
fzf は ghq 以外でも、git の checkout などいろいろな用途に使えるので使ってみてください。
